...
const selectAllChecked = computed(
  () => store.selectionMode === 'filter' || (store.lastEntries.length > 0 && store.lastEntries.every((e) => store.selectedPaths.has(e.path))),
);
...
function onSelectAllChange(event) {
  store.selectionMode = 'explicit';
  for (const entry of store.lastEntries) {
    if (event.target.checked) {
      store.selectedPaths.add(entry.path);
    } else {
      store.selectedPaths.delete(entry.path);
      ...
      <table class="w-full text-sm text-left">
        <thead class="bg-neutral-900 text-neutral-400 uppercase text-xs">
          <tr>
            ...
            <th class="px-4 py-3 cursor-pointer select-none" @click="onSort('path')">Path<span>{{ sortIndicator('path') }}</span></th>
            <th class="px-4 py-3">Format</th>
            <th class="px-4 py-3 cursor-pointer select-none" @click="onSort('duration')">Duration<span>{{ sortIndicator('duration') }}</span></th>
            <th class="px-4 py-3 cursor-pointer select-none" @click="onSort('status')">Status<span>{{ sortIndicator('status') }}</span></th>
            <th class="px-4 py-3">
