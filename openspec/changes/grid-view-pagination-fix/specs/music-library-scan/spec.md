...
- **THEN** selection, bulk actions, opening the details view for a card, and page navigation SHALL behave identically to their table-view equivalents

#### Scenario: Grid view's own pagination controls are functional
- **WHEN** a user is in grid view and uses its own page-navigation controls (not the table view's)
- **THEN** the UI SHALL re-fetch `GET /api/v1/library` with the corresponding `limit`/`offset` and replace the currently rendered cards with that page's results, without requiring the user to switch to table view first
