...
- **THEN** the system SHALL write `TIT2` (title), `TPE1` (artist), `TALB` (album), `TPE2` (album artist), `TRCK` (track number), and `TPOS` (disc number) ID3v2 text frames, an `APIC` frame when cover art is stored, a `USLT` frame when plain lyrics are stored, a `UFID` frame with owner `http://musicbrainz.org` when a recording MBID is resolved, and `TXXX` frames for the release, release-group, and artist MBIDs when those are resolved

#### Scenario: FLAC tagging writes Vorbis comments and a picture block
- **WHEN** tagging is triggered for an identified `.flac` file
- **THEN** the system SHALL write `TITLE`, `ARTIST`, `ALBUM`, `ALBUMARTIST`, `TRACKNUMBER`, and `DISCNUMBER` Vorbis comment fields, a `PICTURE` metadata block when cover art is stored, a `LYRICS` comment field when plain lyrics are stored, and `MUSICBRAINZ_TRACKID`, `MUSICBRAINZ_ALBUMID`, `MUSICBRAINZ_RELEASEGROUPID`, and `MUSICBRAINZ_ARTISTID` Vorbis comment fields for whichever of the recording/release/release-group/artist MBIDs are resolved

#### Scenario: M4A tagging writes MP4 atoms
- **WHEN** tagging is triggered for an identified `.m4a` file
- **THEN** the system SHALL write `©nam`, `©ART`, `©alb`, `aART`, `trkn`, and `disk` MP4 atoms, a `covr` atom when cover art is stored, a `©lyr` atom when plain lyrics are stored, and freeform (`----`) atoms under the `com.apple.iTunes` mean for whichever of the recording/release/release-group/artist MBIDs are resolved
...
#### Scenario: Partial MusicBrainz ID data does not block tagging
- **WHEN** tagging is triggered for an identified file whose tracking record has some but not all of the recording/release/release-group/artist MBIDs resolved
- **THEN** the system SHALL still write the available metadata fields and whichever MBIDs are present, omitting only the tag/frame/atom for each unresolved ID
...
The system SHALL be able to read a file's actual, currently-embedded tag values (title, artist, album, album artist, track number, disc number, year, MusicBrainz recording/release/release-group/artist IDs, whether lyrics are embedded, whether cover art is embedded) directly from the physical file, independent of and without relying on the resolved metadata cached in the tracking store, so that what was actually written can be visually verified against what was resolved.
...
- **THEN** the returned values SHALL match the metadata that was written (title, artist, album, album artist, track number, disc number, year, MusicBrainz recording/release/release-group/artist IDs, and presence of lyrics/cover art)
